...
选择 5.6 模型后，工具会从动态价格目录自动带出模型名和对应价格；自定义模型时可在“高级选项”中手工填写模型名和每百万 Token 价格。README 不重复维护一份固定价格表，避免文档价格与动态数据源发生偏差。
...
在“高级选项”中启用自定义语料后，工具会把用户填写的正文作为固定输入，并继续自动附加“只返回 `BILLING_TEST_OK`”的短格式约束。最大输出 Token 默认是 `16`，它只限制输出上限，不能保证模型恰好输出指定数量的 Token。不要在核验语料中要求随机生成、摘要或改写，否则输出 Token 会产生不必要的波动。
...
- 浏览器历史记录只保存模型、完成时间和计费摘要，不保存 API Key、接口地址或测试语料；可随时在页面中清空。
