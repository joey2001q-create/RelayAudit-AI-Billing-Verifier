  ...
  const providerSections = verification.providers.map((item) => {
    ...
    return `
      <section>
        ...
        <div class="spend"><span>实际消费</span><b>${money(item.actualDeduction)}</b><small>客户填写的平台消费明细金额</small></div>
        <p class="verdict"><strong>${escapeHtml(item.conclusion)}</strong><br>${escapeHtml(item.verdict)}</p>
        <div class="facts">
          <div><span>标称基础费用</span><b>${money(item.standardCost)}</b></div>
          <div><span>按倍率应扣</span><b>${money(item.advertisedExpectedCost)}</b></div>
          <div><span>金额偏差</span><b>${Number.isFinite(item.differenceRate) ? `${money(Math.abs(item.difference))} / ${(Math.abs(item.differenceRate) * 100).toFixed(2)}%` : '待核验'}</b></div>
